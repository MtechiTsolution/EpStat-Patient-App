               ...
               fbAuth.createUserWithEmailAndPassword ( email, password ).addOnCompleteListener ( new OnCompleteListener<AuthResult> () {
                   ...
                   public void onComplete(@NonNull Task<AuthResult> task) {
                       if (task.isSuccessful ()) {
                           ...
                           Toast.makeText ( CreateNewAdmin.this, "1", Toast.LENGTH_SHORT ).show ();
                           UserHelperClass helperClass = new UserHelperClass ( name, username, phone, email, item_gender, designation, password, saveCurrentDate, saveCurrentTime, imageUrl,"sub admin", FirebaseAuth.getInstance ().getCurrentUser ().getUid ());

                           databaseReference.child( "Admin2" ).child ( FirebaseAuth.getInstance ().getCurrentUser ().getUid () ).setValue ( helperClass ).addOnCompleteListener ( new OnCompleteListener<Void> () {
                               @Override
                               public void onComplete(@NonNull Task<Void> task) {

                                   if (task.isSuccessful ()) {

                                       Toast.makeText ( CreateNewAdmin.this, "2", Toast.LENGTH_SHORT ).show ();

                                       Intent intent1=getIntent();
                                       fbAuth.signInWithEmailAndPassword( intent1.getStringExtra( "email" ),intent1.getStringExtra( "password" ) ).addOnCompleteListener( new OnCompleteListener<AuthResult>() {
                                           @Override
                                           public void onComplete(@NonNull Task<AuthResult> task) {
                                               ...
                                               reference.addValueEventListener( new ValueEventListener() {
                                                   ...
                                                   public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                       if (snapshot.child( "Admin2" ).child( FirebaseAuth.getInstance().getCurrentUser().getUid() ).exists()) {
                                                           snapshot = snapshot.child( "Admin2" );
                                                           ...
                                                           String usernameFromDB = snapshot.child( FirebaseAuth.getInstance().getCurrentUser().getUid() ).child( "userame" ).getValue( String.class );
                                                           ...
                                                           String genderFromDB = snapshot.child( FirebaseAuth.getInstance().getCurrentUser().getUid() ).child( "gender" ).getValue( String.class );
                                                           String designationFromDB = snapshot.child( FirebaseAuth.getInstance().getCurrentUser().getUid() ).child( "designation" ).getValue( String.class );
                                                           String imageFromDB = snapshot.child( FirebaseAuth.getInstance().getCurrentUser().getUid() ).child( "url" ).getValue( String.class );

                                                           //create a session
                                                           SessionManager sessionManager = new SessionManager( CreateNewAdmin.this );
